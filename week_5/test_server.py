...
def run(host, port):
    ...
    try:
        ...
        if metrics != expected_metrics:
            print(f"client.get('k2') вернул неверный результат. Ожидается: {}. Получено: {}".format(expected_metrics, metrics))
            sys.exit(1)
    except Exception as err:
    ...
    try:
        ...
        print("Ошибка вызова метода get с ключом, который еще не был добавлен: {} {}".format(expected_metrics, metrics))
        sys.exit(1)
