...
def run(host, port):
    ...
    try:
        ...
        print("Ошибка вызова client.put(...) {}: {}".format(err.__class__, err))
        sys.exit(1)
    ...
    try:
        ...
        if metrics != expected_metrics:
            print("client.get('*') вернул неверный результат. Ожидается: {}. Получено: {}".format(expected_metrics, metrics))
            sys.exit(1)
    except Exception as err:
        print("Ошибка вызова client.get('*') {}: {}".format(err.__class__, err))
        sys.exit(1)
    ...
    try:
        ...
        if metrics != expected_metrics:
            print("client.get('k2') вернул неверный результат. Ожидается: {}. Получено: {}".format(expected_metrics, metrics))
            sys.exit(1)
    except Exception as err:
        print("Ошибка вызова client.get('k2') {}: {}".format(err.__class__, err))
        sys.exit(1)

    try:
        result = client1.get("k3")
        if result != {}:
            print("Ошибка вызова метода get с ключом, который еще не был добавлен. Ожидается: пустой словарь. Получено: {}".format(result))
            sys.exit(1)
    except Exception as err:
        print("Ошибка вызова метода get с ключом, который еще не был добавлен: {} {}".format(err.__class__, err))
        sys.exit(1)
